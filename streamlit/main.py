...
def edit_tables():
    ...
    column_mappings_for_user = {
        "films": ["Название фильма", "Ссылка на фильм"],
        "categorys": ["Название категории"],
        "filmcategorys": ["ID фильма", "ID категории"],
        "users": ["Номер телефона", "Имя", "Фамилия", "Email", "Админ"],
        "directors": ["Имя", "Фамилия",  "Возраст", "Ссылка на фото"],
        "actors": ["Имя", "Фамилия",  "Возраст", "Ссылка на фото"],
    ...
    column_mappings_for_db = {
        "films": ["name", "film_link"],
        "categorys": ["name"],
        "filmcategorys": ["film_id", "category_id"],
        "users": ["phone_number", "first_name", "last_name", "email", "is_admin"],
        "directors": ["name", "surname",  "age", "photo_link"],
        "filmdirectors": ["film_id", "director_id"],
        "actors": ["name", "surname", "age", "photo_link"],
    ...
    if st.button("Загрузить данные"):
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                st.session_state[f"{selected_table}_data"] = pd.DataFrame(data)
            else:
                st.error("Не удалось загрузить данные таблицы.")
        except Exception as e:
            st.error(f"Ошибка: {e}")
